...
function OpenTracingHandler:log(conf)
  ...
  if ctx.authenticated_consumer then
    request_span:set_tag("kong.consumer.id", ctx.authenticated_consumer.id)
    if ctx.authenticated_consumer.username then
      request_span:set_tag("kong.consumer.username", ctx.authenticated_consumer.username)
    end
  end
  ...
  if ctx.service and ctx.service.id then
    proxy_span:set_tag("kong.service.id", ctx.service.id)
    if ctx.route and ctx.route.id then
      proxy_span:set_tag("kong.route.id", ctx.route.id)
    end
    if type(ctx.service.name) == "string" then
      proxy_span:set_tag("peer.service", ctx.service.name)
      proxy_span:set_tag("kong.service.name", ctx.service.name)
    end
    if type(ctx.route.name) == "string" then
      proxy_span:set_tag("peer.route", ctx.route.name)
      proxy_span:set_tag("kong.route.name", ctx.route.name)
    end
